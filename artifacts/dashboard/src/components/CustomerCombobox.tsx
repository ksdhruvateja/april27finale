...
export default function CustomerCombobox({ customers, value, onSelect, onAddNew, required, lightMode = false }: Props) {
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map(c => (
                <button
                  ...
                  style={{
                    background: value === String(c.id)
                      ? (lightMode ? "rgba(59,130,246,0.08)" : "rgba(59,130,246,0.10)")
                      : "transparent",
                    borderBottom: lightMode ? "1px solid #f1f5f9" : "1px solid rgba(255,255,255,0.05)",
                  }}
                  onMouseEnter={e => {
                    if (value !== String(c.id))
                      (e.currentTarget as HTMLElement).style.background = lightMode ? "#f8fafc" : "rgba(255,255,255,0.05)";
                  }}
                  onMouseLeave={e => {
                    (e.currentTarget as HTMLElement).style.background = value === String(c.id)
                      ? (lightMode ? "rgba(59,130,246,0.08)" : "rgba(59,130,246,0.10)")
                      : "transparent";
                  }}
                >
