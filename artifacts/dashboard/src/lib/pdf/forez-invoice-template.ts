...
const DEFAULT_TERMS = [
  ...
  "Estimated Lead Time: 5-7 Business Days, After",
  "Order Confirmation.",
];
...
<div class="top-header">
  <div class="company-block">
    <div class="company-name">${escapeHtml(FOREZ_INVOICE.name)}</div>
    <div>${escapeHtml(FOREZ_INVOICE.line1)}</div>
    <div>${escapeHtml(FOREZ_INVOICE.line2)}</div>
    <div>${escapeHtml(FOREZ_INVOICE.country)}</div>
    <div>${escapeHtml(FOREZ_INVOICE.email)}</div>
    <div>${escapeHtml(FOREZ_INVOICE.website)}</div>
  </div>
  <div class="logo-wrap">
    <img src="${FOREZ_INVOICE.logo}" class="logo" alt="${escapeHtml(FOREZ_INVOICE.name)}" onerror="this.onerror=null;this.src='/favicon.svg';" />
  </div>
  <div class="invoice-title">${escapeHtml(data.invoiceTitle)}</div>
</div>
...
<div class="bottom">
  <div class="terms">
    ${terms.map((p) => `<p class="terms-line">${escapeHtml(data.leadTimeLabel && p.includes("Lead Time") ? data.leadTimeLabel : p)}</p>`).join("")}
    ${notesHtml}
  </div>
  <div class="totals">
    <div class="total-row">
