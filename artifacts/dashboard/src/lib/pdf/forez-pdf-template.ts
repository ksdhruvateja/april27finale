...
/* —— Header: company left, logo center, meta boxes right —— */
.top-header {
  display: grid;
...
<div class="top-header">
  <div class="company-block">
    <div class="company-name">${escapeHtml(FOREZ.name)}</div>
    <div>${escapeHtml(FOREZ.line1)}</div>
    <div>${escapeHtml(FOREZ.line2)}</div>
    <div>${escapeHtml(FOREZ.country)}</div>
    <div>${escapeHtml(FOREZ.email)}</div>
    <div>${escapeHtml(FOREZ.website)}</div>
  </div>
  <div class="logo-wrap">
    <img src="${FOREZ.logo}" class="logo" alt="${escapeHtml(FOREZ.name)}" onerror="this.onerror=null;this.src='/favicon.svg';" />
  </div>
...
export function generateForezLegacyInvoiceHTML(data: InvoiceInput): string {
  return renderForezDocumentHTML({
    ...
    notes: data.notes,
    trackingNumber: data.trackingNumber,
    termsParagraphs: data.termsLines ?? ESTIMATE_TERMS,
  });
}
