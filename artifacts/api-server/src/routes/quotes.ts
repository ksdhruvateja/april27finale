...
router.post("/quotes/:id/convert", async (req, res): Promise<void> => {
  ...
  // When converting, derive invoice number by swapping FRZQ- prefix to FRZI- so they share the same suffix
  const derivedInvoiceNumber = quote.quoteNumber
    ? quote.quoteNumber.replace(/^FRZQ-/, "FRZI-")
    : `FRZI-${await getNextDocNumber()}`;
  const invoiceNumber = req.body?.invoiceNumber ?? derivedInvoiceNumber;
  ...
  await db.update(quotesTable).set({ status: "invoiced" }).where(eq(quotesTable.id, quote.id));
