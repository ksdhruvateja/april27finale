...
export default function VendorModal({ onClose, vendor, onCreated }: Props) {
  ...
  const [emails, setEmails] = useState<string[]>(
    (vendor?.emails as string[] | null) ?? (vendor?.email ? [vendor.email] : [""])
  );
  ...
  const setEmailVal = (i: number, val: string) => setEmails(e => e.map((x, idx) => idx === i ? val : x));
            ...
            {emails.map((em, i) => (
              <div key={i} className="flex gap-2">
                <FormInput
                  ...
                  value={em}
                  onChange={e => setEmailVal(i, e.target.value)}
                />
                {emails.length > 1 && (
