...
router.get("/invoices", async (_req, res): Promise<void> => {
  ...
  res.json(await Promise.all(invoices.map(withCustomerName)));
});
...
router.post("/invoices", async (req, res): Promise<void> => {
  ...
  const [inv] = await db.insert(invoicesTable).values({
    ...
    invoiceNumber: (parsed.data as any).invoiceNumber ?? null,
...
router.patch("/invoices/:id", async (req, res): Promise<void> => {
  ...
  if ((parsed.data as any).invoiceNumber !== undefined) updateData.invoiceNumber = (parsed.data as any).invoiceNumber;
...
router.post("/invoices/:id/pay", async (req, res): Promise<void> => {
  ...
  if (!parsed.success) { res.status(400).json({ error: parsed.error.message }); return; }
  const [inv] = await db.update(invoicesTable).set({ status: "paid", paymentMethod: parsed.data.paymentMethod, paymentNote: parsed.data.paymentNote ?? null, paidAt: new Date() }).where(eq(invoicesTable.id, params.data.id)).returning();
  if (!inv) { res.status(404).json({ error: "Invoice not found" }); return; }
  res.json(await withCustomerName(inv));
});
